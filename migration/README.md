...
The following information is necessary for the migration: 
    ...
    - HOTSHOT_ADDRESS: The address of the hotshot light client contract on the parent chain.
    ...
    - IS_USING_FEE_TOKEN: A boolean representing if this chaing is using a fee token, This is important for constructing 
    the sequencer inbox, and shouldn't be deviated from your current deployment.
...
```
yarn hardhat run scripts/deployEspressoTEEVerifier.ts
```

The following commands, when run from the orbit-actions repo, will run the two forge deployment scripts.
...
```
forge script --chain $CHILD_CHAIN_CHAIN_id contracts/child-chain/espresso-migration/DeployArbOSUpgradeAction.s.sol:DeployArbOSUpgradeAction  --rpc-url $CHILD_CHAIN_RPC_URL --broadcast -vvvv
